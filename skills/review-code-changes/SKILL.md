...
**核心约束**：仅审用户指定范围或最近 diff · 每个问题需指向具体代码 · 不引入新需求/过度重构

## 输入/输出契约
输入：`git diff`/范围、关键规范来源（CLAUDE.md/ESLint）、可用测试命令  
输出：问题清单（维度+位置+建议）+ 测试结果 + 通过/中断结论
...
6. 循环验证：重复 2-5 直到通过；末轮至少 `lint & type-check & test`  
7. 每轮输出：`→ 第{N}轮 ✓真实性 ✓正确性 ✗优雅性:{问题} ✗最小化:{问题}`  
8. 返回结果：`✓ 代码复盘完成，满足所有质量要求` 或 `✗ 复盘中断：{原因}`
