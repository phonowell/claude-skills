...
`pnpm exec tsc` 检查类型 · 逻辑完整性（边界/空值）· 依赖正确性（导入路径/扩展名）；必要时用针对性测试替代全量（但至少跑相关测试）
...
验证策略：优先跑相关测试/目标子集，最终一轮至少执行 `pnpm lint` + `pnpm exec tsc`（再视情况跑关键测试）
...
- [ ] 最终一轮执行 `pnpm lint` + `pnpm exec tsc`，必要测试通过
...
- 第1轮：✓真实性 ✗正确性:缺 `loading?: boolean` → 修复类型 → `pnpm lint + test + tsc` ✓
- 第2轮：✓真实性 ✓正确性 ✗优雅性:未用 `clsx` → 重构 className → `pnpm lint + test + tsc` ✓
- 第3轮：✓真实性 ✓正确性 ✓优雅性 ✗最小化:未使用 `useState` + 深层嵌套 → 移除 + if-return 扁平化 → `pnpm lint + test + tsc` ✓
