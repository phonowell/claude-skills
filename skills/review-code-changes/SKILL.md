...
## 核心意图
通过多轮检查和自动修复，阻断低质量改动进入主分支，并把问题严重度映射到明确动作。
...
7. 动作映射：P0 停止并先修；P1 本轮必须修；P2 修或登记；P3 仅建议
8. 末轮必须说明已执行验证与未执行原因
9. 冲突时优先级：核心约束 > `review-checklist` > 示例；输出必须遵循 `references/output-template.md`
...
4. 自检轮：按 `review-checklist` 执行四维复盘与结构治理
5. 动作轮：按 P0-P3 自动修复并做最小验证
6. 反馈轮：若仍未通过则继续循环复盘
7. 末轮执行 `lint/type-check/test`；不可执行必须注明原因
8. 按输出模板汇总问题、动作、验证与残余风险
