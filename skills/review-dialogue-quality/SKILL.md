...
**前置要求**：Claude Code 中用户需先运行 `/cost` 获取统计数据；其他环境无法调用 CLI 时，从上下文估算（输出 ≈ 输入的 5 倍）或用现有日志
...
**费用**：优先用 `/cost` 实际值，无则估算（输出 ≈ 输入的 5 倍，缺数据时标“估算”）
...
TodoList 未更新（≥3 步骤任务未用 TodoWrite 或 task_plan.md 未更新）· 遗漏功能 · 违反 CLAUDE.md · 未执行检查(tsc/lint)

评分：✅ 完整 · ⚠️ 主要完成或 TodoWrite 遗漏 · ❌ 明显遗漏

## 工作流程

1. **前置检查**：Claude Code 确认有 `/cost` 输出，否则提示运行；其他环境跳过
2. **收集分析**：解析 `/cost`（无则估算并标注）· 工具调用 · 重复读取 · skill 使用 · 任务产出/检查
3. **评分输出**：三维度 ✅/⚠️/❌ + 简洁原因（含数据/估算）
4. **补丁建议**：⚠️/❌ 输出文本建议（不执行 Edit），注明文件/位置/建议
5. **返回信息**：输出 "✓ 检查完成" 或 "✗ 中断：{原因}"；返回前自检评分/建议格式

## 验证清单
- [ ] 费用来源明确（/cost 或估算已标注）
- [ ] 工具规范问题已列出（如重复读/错误工具选择/未并行）
- [ ] 任务完成度核对（TodoWrite 或 task_plan.md 更新，检查/验证情况）
- [ ] 补丁建议包含文件/位置/建议，未执行 Edit
- [ ] 返回信息格式正确（含评分/建议）
