...
6. 必保留原则：Skill 使用等待完成；总是要保证最小心智负担；精简冗余·冲突信代码；客观诚实；`try-catch` 仅高 ROI；≥3 步任务用 `/plans/task_plan_{suffix}.md`；输出格式约束；`AGENTS.md` 必须创建且无需询问用户，并固定使用 `syslink`（`symlink`）与 `CLAUDE.md` 链接
7. Token 控制：输出长度 ≤ 输入*5，优先符号压缩 `· → /`
8. 失败即中断：找不到文件、关键段冲突、验证未过时返回 ✗

## 输入/输出契约

- 输入：`CLAUDE.md` 路径、保留段、关键命令/路径
...
8. 强制创建：若缺失则创建 `AGENTS.md`，无需询问用户；内容仅保留指向 `CLAUDE.md` 的 `syslink`（`symlink`）与链接
9. 验证：`wc -l CLAUDE.md` ≤100；必保留段齐全；命令/路径/链接有效；`AGENTS.md` 存在且链接正确
10. 返回：仅在验证通过后输出 `✓ 已优化至 {行数} 行`，否则 `✗ 中断：{原因}`
...
- [ ] 必保留原则齐全（Skill 使用/最小心智负担/元原则/客观诚实/异常处理/计划管理/输出约束/`AGENTS.md` 强制创建与 `syslink` 链接）
- [ ] 返回信息格式正确
- [ ] 命令/路径/链接可执行或引用有效（含 `AGENTS.md -> CLAUDE.md`）
