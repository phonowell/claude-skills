...
2. Read + `wc -l` 并行获取内容/行数
3. 列保留段：核对保留段是否缺失/过时，不要删可执行命令/路径/约束条目
4. 识别冗余：通用知识 · 重复表达 · 解释性文字 · 低频场景
5. 应用压缩：符号替代 · 合并配置 · 代码引用 · 删除助词；保持指令可执行、链接/路径有效
6. 保留必备原则（如 CLAUDE.md 所列）：
   - Skill 使用：调用后等待完成再执行
   - 元原则：`精简冗余 · 冲突信代码`
...
7. Edit 文件：一次性完成所有修改
8. 验证：`wc -l` ≤100 → 检查必保留段齐全且内容最新 → 验证清单逐项
9. 返回信息（未验证勿返回）：输出 "✓ 已优化至 {行数} 行" 或 "✗ 中断：{原因}"
