...
6. 必保留原则：Skill 使用等待完成；精简冗余·冲突信代码；客观诚实；`try-catch` 仅高 ROI；≥3 步任务用 `/plans/task_plan_{suffix}.md`；输出格式约束；`AGENTS.md` 必须创建且无需询问用户，并固定使用 `syslink`（`symlink`）与 `CLAUDE.md` 链接
...
- [ ] 必保留原则齐全（Skill 使用/元原则/客观诚实/异常处理/计划管理/输出约束/`AGENTS.md` 强制创建与 `syslink` 链接）
- [ ] 返回信息格式正确
