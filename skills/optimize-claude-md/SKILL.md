...
1. **确认需求**：**调用 `plan-implementation` skill**（强制）
2. **Read + wc -l** 并行获取内容和行数
3. **识别冗余**：通用知识 · 重复表达 · 解释性文字 · 低频场景
4. **应用压缩**：符号替代 · 合并配置 · 代码引用 · 删除助词
5. **确保必备原则**：
...
6. **Edit 文件**：一次性完成所有修改
7. **验证**：`wc -l` 确认 ≤100 → 执行验证清单逐项检查
8. **返回信息**：输出 "✓ 已优化至 {行数} 行" 或 "✗ 中断：{原因}"
