...
**简单任务直接执行**：单文件+明确需求+无架构决策 → 提示"任务较简单，建议直接执行（节省 ~10K tokens）· 继续计划？"

**委托评估**：≥2 文件修改/批量操作/独立子任务 → **优先使用 `invoke-opencode-acp` skill 委托**（默认委托，~6s 开销可接受，仅单文件快速修改直接执行）
...
**文件路径约束**：必须创建在**项目 `/plans` 目录**下，禁止创建在 skill 目录

**核心循环**（≥3步骤任务强制执行）：

1. Write `/plans/task_plan.md`（初始化，项目 plans 目录）
...
**时机**：大量探索前，避免走错方向浪费 tokens · `AskUserQuestion` 列 2-3 方案 · **每项必须说明优缺点** · **首项为推荐选项并标注**
...
**时机**：计划制定后、执行前，确保用户认可最终方案 · `AskUserQuestion` 展示概要 · **每项说明优缺点** · **首项为推荐方案并标注** · **等确认**后执行
...
严格按计划 · **≥3步骤强制 task_plan.md**（<3步可用 TodoWrite） · **批量操作模式**（批量 Read → 批量 Edit → 单次验证）· Edit 无依赖批量调用 · 需调整立即中断 `AskUserQuestion` · 失败分析根因回退（同文件≥3次停止）

**重构检查**（改签名/类型/参数时）：标注测试契约点 · 评估改动成本 vs 收益 · 保守优化 > 激进重构 · 失败立即回滚
