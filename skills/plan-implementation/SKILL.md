...
**效率优先**：Read/Grep/Glob > Task · ≥3 步骤用 /plans/task_plan_{suffix}.md（替代 TodoWrite，持久化优先）
...
### 2. 计划载体（≥3 步任务）
- 3 文件模式：`/plans/task_plan_{suffix}.md`（目标/阶段/决策/错误/状态）；`/plans/notes_{suffix}.md`（研究数据）；`[deliverable].md`（交付物）
- 独立 task 必须使用唯一 {suffix}（建议 task slug）；不得混用/复用计划与 notes，避免冲突
- 命名示例：`task_plan_user-sync.md` + `notes_user-sync.md`（suffix=`user-sync`）
- 路径约束：文件放 `/plans`，禁止放 skill 目录
- 核心循环（≥3 步强制）：初始化 task_plan_{suffix}.md → 每轮前 Read → 每轮后 Edit 记录进度/错误/决策 → 禁用 TodoWrite 替代（<3 步可用 TodoWrite）
- 注意力钩子：未更新 task_plan_{suffix}.md 不得进入下一轮；对话中形成的假设/决策/阻塞，随手写入 notes_{suffix}.md（避免遗忘）
...
- 结构建议：分层计划（目标→子任务→步骤）+ 计划-执行-复盘闭环
- 协作建议：必要时拆分子任务并行/委托；复杂任务可同步外部任务板/清单

### 6. 确认计划（执行前）
- 用 `AskUserQuestion` 展示概要和优缺点，首项推荐；等确认后执行

### 7. 执行实现
- 执行前：Read task_plan_{suffix}.md 刷新目标/阶段
- 按计划执行；需调整时先中断并确认；失败同文件≥3 次停止分析
- 重构检查：改签名/类型/参数时标注契约点，保守优先
- 阶段完成：Edit task_plan_{suffix}.md 记录进度/错误/决策，补充 notes_{suffix}.md 中的对话要点/假设/风险

### 8. 返回信息
- 输出 `✓ 已完成` / `✓ 计划已确认` / `✓ 直接执行` / `✗ 中断：{原因}`
- 返回前自检：task_plan_{suffix}.md 已更新最新进度/错误/决策，notes_{suffix}.md 已写关键对话/假设/风险
